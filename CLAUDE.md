...
- **LLM loading** (notebook 05): Use `AutoModelForCausalLM` with `dtype=torch.float16` (not `torch_dtype=`, which is deprecated) for smaller models (Phi-3). Use `BitsAndBytesConfig(load_in_4bit=True)` for 7B+ models (Mistral). Do NOT use `trust_remote_code=True` for Phi-3 — it is natively supported in transformers 4.57+ and the remote custom code is outdated (references removed `DynamicCache.seen_tokens`).
